...
public class JdbcCityDao implements CityDao {
    ...
    public City getCityByNameAndState( String cityName, String stateAbbreviation ) {

        City city = null;
        String sql = "SELECT city_id, city_name, state_abbreviation, population, area " +
                "FROM city " +
                "WHERE state_abbreviation = ? AND city_name= ?;";
        SqlRowSet results = jdbcTemplate.queryForRowSet(sql, stateAbbreviation, cityName);
        if (results.next()) {
    ...
    @Override
    public List<City> getCityListByStateAbbreviation(String stateAbbreviation) {
